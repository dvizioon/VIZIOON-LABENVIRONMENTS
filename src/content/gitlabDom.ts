...
function getVariablesRoot(): Element | null {
  return (
    document.querySelector('#secret-variables') ??
    document.querySelector('section.qa-variables-settings') ??
    document.querySelector('.js-ci-variable-list-section')
  )
}

function getVariableList(): HTMLUListElement | null {
  const root = getVariablesRoot()
  if (!root) return null
  return root.querySelector('ul.ci-variable-list')
}
...
export function detectVariablesPage(): PageContext {
  ...
  const saveEndpoint =
    ...
    getVariablesRoot()?.getAttribute('data-save-endpoint') ??
    undefined
...
export async function importVariablesToPage(
  ...
  if (!page.ready) {
    return {
      success: false,
      imported: 0,
      failed: [{ key: '*', error: 'Página de variáveis não encontrada' }],
    }
  }
