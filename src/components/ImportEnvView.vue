...
const { pageContext, checking, refresh, clearPageVariables, applyQueueToPage } = useApplyQueue()
...
async function onApplyQueueEvent() {
  await applyQueueToPage()
}

onMounted(async () => {
  await Promise.all([envTabs.load(), historyStore.load()])
  loadTabQueue()
  window.addEventListener('vizioon:apply-queue', onApplyQueueEvent)
})

onUnmounted(() => {
  window.removeEventListener('vizioon:apply-queue', onApplyQueueEvent)
})
...
function readFile(file: File) {
  ...
  reader.onload = async (e) => {
    ...
    if (importStore.parseErrors.length) {
      ...
      show(`${importStore.pairCount} variáveis em ${prefix || 'sem prefixo'}`, 'success')
    }
  }
...
async function clearAllOnPage() {
  ...
  try {
    ...
    const msg = result.saved
      ...
      : `${result.cleared} marcadas para remoção. Clique em Save variables`
    show(msg, 'success')
  } catch (err) {
...
<template>
  <div class="vz-main">
      ...
      <div class="vz-status__text">
        <strong>
          ...
          <template v-else-if="pageContext.ready">
            {{ pageContext.title || 'GitLab' }}
          </template>
          <template v-else>GitLab</template>
        </strong>
        ...
        <span v-else>Variáveis não detectadas</span>
      </div>
    ...
    <div class="vz-section">
      ...
      <div class="vz-list">
        <div v-if="!pageContext.ready" class="vz-empty">Nenhuma variável visível</div>
