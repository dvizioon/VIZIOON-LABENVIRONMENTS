...
const onCiCd = ref(false)
const pageChecked = ref(false)
const showGitlabHelp = ref(false)
...
async function openPanel(tab?: string) {
  ...
  try {
    ...
    if (res?.error === 'reloadPage') {
      statusMsg.value = 'Recarregue esta aba (F5) e tente de novo.'
...
async function detectCiCdTab() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true })
    return isCiCdSettingsUrl(tab?.url)
  } catch {
    return false
  }
}
...
onMounted(async () => {
  const [gitlab] = await Promise.all([
    detectCiCdTab(),
    settings.load(),
  ])
  onCiCd.value = gitlab
...
<template>
  <div class="vz-popup" :class="{ 'vz-popup--privacy': showPrivacyModal }">
    ...
    <template v-else-if="pageChecked && !onCiCd">
      <div class="vz-popup-notice">
        <p>GitLab não encontrado</p>
        <button type="button" class="vz-popup-notice__more" @click="showGitlabHelp = !showGitlabHelp">
          <VzIcon icon="ph:info-fill" :size="16" />
          Saiba mais
        </button>
        <p v-if="showGitlabHelp" class="vz-popup-notice__help">
          O GitLab não foi encontrado nesta aba. Esta extensão foi feita para variáveis.
        </p>
      </div>
    </template>

    <template v-else-if="onCiCd">
