export function mountGitlabVariablesPage(options?: {
  ...
  const legacyLayout = options?.legacyLayout ?? false
  ...
  const sectionAttrs = legacyLayout
    ? 'class="qa-variables-settings settings expanded"'
    : `id="secret-variables" class="js-ci-variable-list-section" data-save-endpoint="${saveEndpoint}"`

  const listWrapper = legacyLayout
    ? `<div class="col-lg-12 js-ci-variable-list-section" data-save-endpoint="${saveEndpoint}">`
    : ''
  const listWrapperClose = legacyLayout ? '</div>' : ''

  document.body.innerHTML = `
    <div class="sidebar-context-title">${title}</div>
    <section ${sectionAttrs}>
      ${legacyLayout ? listWrapper : ''}
